...
export const beginPostCreation = {
  // This endpoint is for creating jobs via the REST API
  path: '/begin-post-creation',
  method: 'post',
  handler: async (req: PayloadRequest) => {
    if (typeof req.json !== 'function') {
      throw new Error('[/begin-post-creation] Request has no JSON method')
    }
    ...
    if (task && workflow) {
      throw new Error('[/begin-post-creation] Cannot queue both a task and a workflow')
    }
    ...
    setImmediate(async () => {
      try {
        ...
        console.error(`[/begin-post-creation] Job '${job.id}' failed:`, error)
      }
    })
